...
exports.editQuestion = (req, res) => {
    Question.find({ _id: req.body.questionid }, (err, q) => {
        if (err) {
            console.log(error);
            res.json({ msg: "some error!" });
        }
        else {
            Question.updateOne({ questionText: req.body.questionText },
                { answer: req.body.answer },{options: req.body.options}, function (err, user) {
                    console.log(1);
                    if (err) {
                        console.log(err)
                        res.json({ msg: "Somthing went wrong" });
